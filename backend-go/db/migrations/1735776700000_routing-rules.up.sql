...
-- product. The evaluator (a later phase) always looks at both:
-- product-specific rules AND entity-wide rules, ordered together by
-- priority (first-match-wins across the combined set).
--
...
-- breaker is open — before falling through to the next
-- lower-priority rule entirely. Nullable: not every rule needs a
-- same-rule fallback.
--
-- routing_rules_audit is append-only (mirrors payment_events/
-- transactions/audit_log's pattern) — the "audit trail" requirement
-- for T5.1 means every create/update/delete of a rule is a permanent,
