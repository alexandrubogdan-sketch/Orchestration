...
-- instrument belongs to — every MIT attempt must carry the
...
-- Re-routing a subscription to a different PSP account (Milestone 5's
-- routing_rules) is out of scope here — it would require re-tokenizing
-- the instrument, which is a customer-facing card-migration flow, not
-- something a renewal cron can do silently.
--
-- dunning_stage/dunning_next_retry_at live on the subscription itself
-- rather than a separate table — T8.2's dunning ladder is a property
-- of one subscription's retry history, not an independent entity with
-- its own lifecycle.
