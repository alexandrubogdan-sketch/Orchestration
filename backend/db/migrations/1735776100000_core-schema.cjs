 ...
 *
 * SQL-first per ADR-0001 — everything here is raw DDL via pgm.sql()
 * rather than the table-builder API, because half of what this
 * migration needs to express (CHECK constraints tied to the canonical
 * state machine, a trigger-enforced append-only rule, partial unique
 * indexes) doesn't have a clean builder-API equivalent, and mixing
 * builder calls with raw SQL in one migration is harder to read than
 * committing to one style.
 *
 * Primary keys are `uuid` columns populated by the application (see
 * db/migrations/1735776000000_init-extensions.cjs for why UUIDv7 is
 * generated in app code, not via a DB extension/default) — so no
 * column here has a `DEFAULT gen_random_uuid()` or similar; every
 * insert must supply an id.
 *
 * State/category/type enums are CHECK constraints on `text` columns,
 * not native Postgres ENUM types — enum types require a migration to
 * add new values and complicate Kysely's generated types across
 * versions; a CHECK constraint is just as strict and is a one-line
...
exports.up = (pgm) => {
  pgm.sql(`
    ...
    -- Partial unique index: external_ref only has to be unique per
    -- entity when a product actually supplies one.
    CREATE UNIQUE INDEX customers_entity_external_ref_key
      ON customers(merchant_entity_id, external_ref)
      WHERE external_ref IS NOT NULL;

    -- Maps a customer to their PSP-side customer id per psp_account.
    -- Needed because a payment method vaulted at PSP A is not routable
    -- to PSP B (docs/design.md §1.4 / SPEC.md Milestone 5 note) —
    ...
    -- Token refs only — no PAN/CVV, ever (Non-negotiable #8). Card
    -- brand/last4/expiry are display-safe metadata PSPs return
    -- alongside the token, not sensitive data on their own.
    ...
    -- src/domain/stateMachine.ts (PAYMENT_STATES) and docs/design.md §2.
    -- If you add a state to one, add it to both others in the same PR.
    ...
    CREATE TABLE payment_attempts (
      ...
      -- (Non-negotiable #4). Deterministic per attempt.
    ...
    -- Append-only ledger (Non-negotiable #6). UPDATE/DELETE are blocked
    -- by trigger below, not just by convention.
    ...
    -- event_type carries both canonical state-machine event types and
    -- the two special timeline markers ('late_event',
    -- 'invariant_violation') recorded by src/domain/stateMachineDb.ts.
    ...
    -- bind the table owner (Postgres owners implicitly bypass grants),
    -- and this project's single dev/CI DB role IS the table owner. A
    -- BEFORE trigger that raises unconditionally is enforced for every
    -- role, including the owner, which is what "never updated or
    -- deleted" (Non-negotiable #6) actually requires here. In
    -- production, pair this with a dedicated non-owner app role and
    -- REVOKE as defense in depth (see docs/adr/0003-secrets-management.md
    -- for the broader prod-hardening posture) — the trigger is the
    -- floor, not a replacement for least-privilege roles.
    ...
    -- client -> API idempotency (Non-negotiable #4). 'key' alone is
    -- the natural key per SPEC.md T1.1 ("idempotency_keys (unique
    -- key)"); product_id is carried for audit/debugging, not scoping,
    -- so a key collision across products is still caught rather than
    -- silently allowed.
    ...
    -- Transactional outbox (T1.6): inserted in the same transaction as
    -- the domain write it describes, relayed at-least-once by a Hatchet
    -- task (src/workflow/tasks/outboxRelay.ts).
    CREATE TABLE outbox (
      id uuid PRIMARY KEY,
    ...
    -- Decline taxonomy (T1.4 / docs/design.md §3). raw_code preserves
    -- the PSP's original code alongside our normalization.
